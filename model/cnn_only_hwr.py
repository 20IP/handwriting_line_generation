...
class CNNOnlyHWR(nn.Module):
    ...
    def __init__(self, nclass, nc=1, cnnOutSize=512, nh=512, leakyRelu=False, norm='group',  small=False, pad=True):
        ...
        ks = [3, 3, 3, 3, 3, 3, 3]
        # ks = [2, 2, 2, 2, 2, 2, 2] # temp
        ps = [1, 1, 1, 1, 1, 0, 0]
        # ps = [2, 2, 2, 2, 2, 2, 2] # temp
        ss = [1, 1, 1, 1, 1, 1, 1]
        # ss = [1, 2, 2, 2, 2, 2, 2] # temp
    ...
    def forward(self, input_cn, style=None):
        ...
        # ic(input_cn.shape)
        # print("Input shape after padding:", input_cn.shape) #temp
        # conv features
        conv = self.cnn(input_cn)
        # print(type(conv))
        b, c, h, w = conv.size()
        # print("Conv output shape:", conv.shape) #temp
        # assert h == 1, "the height of conv must be 1"
        conv = conv.view(b, -1, w)
        # conv = conv.reshape(b, -1, w) # temp
        # print("Conv shape after view:", conv.shape) #temp
        conv = self.cnn1d(conv)
        # print("Conv shape after cnn1d:", conv.shape) #temp
        output = conv.permute(2, 0, 1)  # [w, b, c]
        # print("Output shape:", output.shape) #temp
    ...
    def setup_save_features(self):
        ...
        def factorySave(i):
            def saveX(module, input_cn ,output):
